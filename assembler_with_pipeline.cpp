...
void instructionExecution(IDEX *idex, EXMO *exmo, MOWB *mowb, PipelineControl *ctrl)
{
    ...
    if (!idex->valid)
    {
        cout << "Invalid instruction in EX stage, skipping..." << endl;
        return;
    }
    ...
    if (idex->CW.Jump && idex->opcode == "1101111")
    {
        ...
        exmo->ALUOUT = idex->DPC + 4; // Store return address
        exmo->rdl = idex->rdl;
        PC = idex->JPC;
    ...
    { // S-type (store)
        ...
        exmo->rs2 = idex->rs2; // Store value to be written to memory
    }
    else if (idex->opcode == "1100011")
    { // B-type (branch)
        cout << "Branch instruction detected" << endl;
        string imm = (idex->imm2).substr(0, 1) + (idex->imm2).substr(11, 1) +
                     (idex->imm2).substr(1, 6) + (idex->imm2).substr(7, 4);

        int comparison = ALU(ALUSelect, idex->rs1, idex->rs2);
        cout << "Branch comparison result: " << comparison << endl;

        // Determine if branch should be taken
        switch (idex->func)
        {
        case 0: // beq
            branch_taken = (comparison == 0);
            break;
        case 1: // bne
            branch_taken = (comparison != 0);
            break;
        ...
        if (branch_taken)
        {
            int offset = SignedExtend(imm);
            PC = idex->DPC + offset * 4;
    ...
    { // R-type and I-type
        ...
        cout << idex->rs1 << idex->rs2 << endl;
        exmo->ALUOUT = ALU(ALUSelect, idex->rs1, idex->rs2);
    }

    // Update EXMO stage registers
    exmo->CW = idex->CW;
    exmo->rdl = idex->rdl;
    exmo->valid = true;
...
void instructionFetch(IFID *ifid, PipelineControl *ctrl)
{
    ...
    if (ctrl->stall)
    {
        cout << "Pipeline stalled, maintaining current instruction" << endl;
        return;
    }

    if (ctrl->flush_if)
    {
        cout << "Pipeline flush detected - inserting NOP" << endl;
    ...
    if (PC >= 0 && PC / 4 < instructionMemory.size())
    {
        ...
        cout << "Fetched instruction from address " << PC << ": " << ifid->IR << endl;
        PC = PC + 4;
    }
    else
    {
...
void instructionDecode(IFID *ifid, IDEX *idex, MOWB *mowb, EXMO *exmo, PipelineControl *ctrl)
{
    ...
    idex->rdl = stoi(instruction.substr(20, 5), nullptr, 2);
    idex->CW = controlUnit[instruction.substr(25, 7)];
    idex->func7 = stoi(instruction.substr(0, 7), nullptr, 2);
    idex->opcode = instruction.substr(25, 7);
    idex->rsl1 = stoi(instruction.substr(12, 5), nullptr, 2);
    idex->rsl2 = stoi(instruction.substr(7, 5), nullptr, 2);
...
int main()
{
    ...
    cout << " Manual input\n";
    cout << " Use default assembly instructions\n";
    ...
    MOWB mowb = {};
    int cycle = 0;
    const int MAX_CYCLES = 100; // Safety limit to prevent infinite loops

    while ((PC < instructionMemory.size() * 4 || ifid.valid || idex.valid || exmo.valid || mowb.valid) && cycle < MAX_CYCLES)
    {
        cout << "\n<======================= Cycle " << ++cycle << " =======================>" << endl;
        ...
        if (idex.valid)
        {
            instructionExecution(&idex, &exmo, &mowb, &ctrl);
            idex.valid = false;
        }

        // ID stage
        if (ifid.valid && !ctrl.stall)
        {
        ...
        // IF stage
        if (!ctrl.stall && PC < instructionMemory.size() * 4)
        {
            instructionFetch(&ifid, &ctrl);
        }

        // Check for completion
        if (PC >= instructionMemory.size() * 4 && !ifid.valid && !idex.valid &&
            !exmo.valid && !mowb.valid)
        {
            break;
        }
    }
    ...
    if (cycle >= MAX_CYCLES)
    {
        cout << "Warning: Maximum cycle limit reached. Possible infinite loop detected." << endl;
    }

    cout << "\nProgram completed in " << cycle << " cycles." << endl;
